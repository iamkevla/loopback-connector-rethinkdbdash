...
var createAllPromise = function(model, filter) {
    ...
    if(filter.index) {
        promise = buildIndex(_this, model, filter.index, promise);
    }
...
RethinkDB.prototype.destroyAll = function destroyAll(model, where, callback) {
    ...
    if(where !== undefined) {
        // check if where object keys are a secondary index
        // if so build index
        let whereKeys = Object.keys(where);
        for(let key of whereKeys) {
            let hasIndex = _hasIndex(_this, model, key);
            if(hasIndex && where[key] !== Object(where[key])) {
                promise = buildIndex(_this, model, {[key]: where[key]}, promise);
                // delete where[key];
                break;
            }
        }
...
function buildIndex(self, model, index, promise) {

    if(index === null || (typeof index !== 'object') || Object.keys(index).length !== 1 || !_hasIndex(self, model, Object.keys(index)[0])) {
        return promise;
    }
    ...
    if(getAllParams) {
        return promise.getAll(...getAllParams);
    } else {
        return promise;
    }
}
